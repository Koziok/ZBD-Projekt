    ...
    <content>
        ...
        <div class="php-script">           
            <?php
                ...
                $query1="SELECT nazwa_silnika from silniki order by nazwa_silnika";
                $result1=mysqli_query($conn, $query1);
                while($row1=mysqli_fetch_assoc($result1))
				{		
					echo '<option value="'.$row1['nazwa_silnika'].'">'.$row1['nazwa_silnika'].'</option>';
				}
                ...
                $check_query1="SELECT nazwa_silnika FROM gry where nazwa_silnika='$choose'";
                $check_result1=mysqli_query($conn, $check_query1);
                $check1=mysqli_num_rows($check_result1);
                $check_query2="SELECT nazwa_silnika FROM wsparcia where nazwa_silnika='$choose'";
                $check_result2=mysqli_query($conn, $check_query2);
                $check2=mysqli_num_rows($check_result2);

                if ($check1 == 0 && $check2 == 0)
                {
                    if ($choose != 'Wybierz silnik...' && strlen($choose) > 1) {
                        $query3="DELETE from silniki where nazwa_silnika='$choose'";
